...
struct Chunklist {
    ...
    init(from url: URL, size: Int) throws {
        ...
        guard data.count == size else {
            throw MistError.chunklistValidationFailed("Invalid file size: '\(data.count)', should be '\(size)'")
        }
        ...
        guard magicHeader == Chunklist.magicHeader else {
            throw MistError.chunklistValidationFailed("Invalid magic header: '\(magicHeader.hexString())', should be '\(Chunklist.magicHeader.hexString())'")
        }

        guard headerSize == Chunklist.headerSize else {
            throw MistError.chunklistValidationFailed("Invalid header size: '\(headerSize.hexString())', should be '\(Chunklist.headerSize.hexString())'")
        }

        guard fileVersion == Chunklist.fileVersion else {
            throw MistError.chunklistValidationFailed("Invalid file version: '\(fileVersion.hexString())', should be '\(Chunklist.fileVersion.hexString())'")
        }

        guard chunkMethod == Chunklist.chunkMethod else {
            throw MistError.chunklistValidationFailed("Invalid chunk method: '\(chunkMethod.hexString())', should be '\(Chunklist.chunkMethod.hexString())'")
        }

        guard signatureMethod == Chunklist.signatureMethod else {
            throw MistError.chunklistValidationFailed("Invalid signature method: '\(signatureMethod.hexString())', should be '\(Chunklist.signatureMethod.hexString())'")
        }

        guard padding == Chunklist.padding else {
            throw MistError.chunklistValidationFailed("Invalid padding: '\(padding.hexString())', should be '\(Chunklist.padding.hexString())'")
        }

        guard chunksOffset == Chunklist.chunksOffset else {
            throw MistError.chunklistValidationFailed("Invalid chunks offset: '\(chunksOffset.hexString())', should be '\(Chunklist.chunksOffset.hexString())'")
        }

        guard chunks.count == totalChunks else {
            throw MistError.chunklistValidationFailed("Incorrect number of chunks: '\(chunks.count)', should be '\(totalChunks)'")
        }

        guard signatureOffset == chunksOffset + totalChunks * 36 else {
            throw MistError.chunklistValidationFailed("Invalid signature offset: '\(signatureOffset.hexString())', should be '\((chunksOffset + totalChunks * 36).hexString())'")
        }
    }
